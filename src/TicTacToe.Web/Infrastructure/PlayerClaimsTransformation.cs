...
public class PlayerClaimsTransformation : IClaimsTransformation
{
    ...
    public PlayerClaimsTransformation(IPlayerRepository players, ILogger<PlayerClaimsTransformation> logger)
    {
        _players = players;
        _logger = logger;
    }
    ...
    public async Task<ClaimsPrincipal> TransformAsync(ClaimsPrincipal principal)
    {
        ...
        {
            _logger.LogDebug("Adding email claim for player {PlayerId}: {Email}", playerId.Value, player.Email);
            claims.Add(new Claim(ClaimTypes.Email, player.Email));
        }
        else
        {
            _logger.LogDebug("No email available for player {PlayerId}", playerId.Value);
        }
